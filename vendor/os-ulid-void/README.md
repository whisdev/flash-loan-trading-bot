...
To quickly generate a ULID, you can simply import the `os-ulid-void` function:

```typescript
import { ulid } from "os-ulid-void";
...
```typescript
import { monotonicFactory } from "os-ulid-void";
...
`os-ulid-void` automatically detects a suitable (cryptographically-secure) PRNG. In the browser it will use `crypto.getRandomValues` and on NodeJS it will use `crypto.randomBytes`.

#### Using `Math.random` (insecure)

By default, `os-ulid-void` will not use `Math.random` to generate random values. You can bypass this limitation by overriding the PRNG:
...
```typescript
import { isValid } from "os-ulid-void";
...
```typescript
import { decodeTime } from "os-ulid-void";
...
```typescript
import { encodeTime } from "os-ulid-void";
...
`os-ulid-void` can be used on the command line, either via global install:

```shell
npm install -g os-ulid-void
ulid
```
...
You can test `os-ulid-void`'s performance by running `npm run bench`:
